  ...
  async getKasStatus(outlets) {
    ...
    for (const outlet of outlets) {
      try {
        ...
        // Cek Starting Cash shift berikutnya
        ...
        if (nextReport && kasAkhir > 0) {
          // Hitung setoran yang terjadi SETELAH laporan terakhir dan SEBELUM laporan berikutnya
          ...
          if (Math.abs(selisihStarting) > 0) {
            startingCashWarning = {
              expected: expectedStarting,
              actual: actualStarting,
              selisih: selisihStarting,
              shift: nextShift,
              tanggal: nextTanggal,
              adaSetoran: totalSetoranAntarShift > 0,
              totalSetoran: totalSetoranAntarShift
            };
          }
        }
